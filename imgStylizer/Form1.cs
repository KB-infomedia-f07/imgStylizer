namespace imgStylizer
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            for (int j = 0; j < 5; j++)
            {
                Bitmap bestGenerationImg = new Bitmap(originalImg.Width, originalImg.Height);
                ...
                for (int i = 0; i < 100; i++)
                {
                    ...
                    //generate color
                    int colorIndex = rand.Next(imgColors.Count);
                    var pixelcolor = imgColors[colorIndex];
                    int r = pixelcolor.R;
                    int b = pixelcolor.B;
                    int g = pixelcolor.G;
                    ...
                    for (int x = 0; x < newImg.Width; x++)
                    {
                        for (int y = 0; y < newImg.Height; y++)
                        {
                            ...
                            score += (int)(Math.Pow(Math.Abs(origianlPixel.R - r), 3) + Math.Pow(Math.Abs(origianlPixel.B - b), 3) + Math.Pow(Math.Abs(origianlPixel.G - g), 3));
                        }
                    }
                    if (score < bestScore)
                    {
